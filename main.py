...
async def handle_message(message: types.Message):
    ...
    # Проверяем, ожидаем ли ответ "да/нет" от пользователя
    ...
    async with LinkChecker() as checker:
        ...
        if client_record and client_record[4]:  # Если клиент уже забронирован
            if client_record[4] == user_username:
                await message.answer(
                    f"🔴 Этот клиент уже занят <b>вами</b>!\n\n Отправьте другого клиента.",
                    parse_mode="HTML"
                )
            else:
                await message.answer(
        ...
        elif client_record and not client_record[4]:  # Если клиент есть в базе, но свободен
            # Сохраняем данные для ожидания ответа
